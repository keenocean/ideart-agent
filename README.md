...
Blog posts are published from **Admin → Posts** and rendered server-side from
the database. Keep article images in R2/S3. Local Markdown/MDX may be used as
an editorial source, but application code must never import it: this keeps new
articles out of the Cloudflare Worker bundle while preserving crawlable HTML,
canonical/hreflang metadata, `BlogPosting` JSON-LD, and sitemap discovery.
`pnpm blog:check-bundle` enforces that boundary after every production build.
...
| `pnpm skills:build`   | Build an immutable local Skill release     |
| `pnpm skills:publish` | Publish and verify a Skill release in R2   |
