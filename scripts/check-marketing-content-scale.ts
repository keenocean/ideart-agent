...
import { marketingContentPageKeys } from '@/content/marketing';

const fixtureCount = 100;
const root = await mkdtemp(path.join(tmpdir(), 'marketing-content-scale-'));

if (!marketingContentPageKeys.some((key) => key.startsWith('tool:'))) {
  const indexBytes = (
  ...
  console.log(
    `Marketing scale check skipped: no published tool pages; runtime index=${indexBytes} bytes.`
  );
  process.exit(0);
}
