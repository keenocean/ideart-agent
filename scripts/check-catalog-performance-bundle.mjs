...
const MAX_RENDER_BLOCKING_CSS_BYTES = 175 * 1024;
const MAX_RENDER_BLOCKING_CSS_GZIP_BYTES = 26 * 1024;
...
if (fontFiles.length > 0) {
  throw new Error(
    `Catalog performance check failed: emitted network fonts: ${fontFiles
...
for (const file of cssFiles) {
  ...
  if (/@font-face|fonts\.(?:googleapis|gstatic)\.com/.test(source)) {
    throw new Error(
      `Catalog performance check failed: ${path.basename(file)} introduces a font dependency chain`
    );
  }
...
console.log(
  `Catalog performance bundle check passed: ${cssFiles.length} inlined CSS file(s), ${cssBytes} raw / ${cssGzipBytes} gzip bytes, ${catalogDetailJsGzipBytes} initial detail JS gzip bytes, 0 network fonts.`
);
