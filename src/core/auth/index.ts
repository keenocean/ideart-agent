...
import { signupBlockedForIp } from './signup-guard';
      ...
      user: {
        create: {
          before: async (userData: any, ctx: any) => {
            ...
            const ip = getClientIpFromCtx(ctx).slice(0, 100);

            // Signup hands out credits, so cap how many accounts one address
            // may open per day. Checked here rather than on the route so it
            // covers social logins too — they create users without ever
            // touching the sign-up endpoint.
            const configs = await getAllConfigs().catch(() => null);
            if (configs && (await signupBlockedForIp(ip, configs))) {
              throw new APIError('TOO_MANY_REQUESTS', {
                message: 'Too many accounts created from this address today.',
              });
            }
