...
export class R2Provider implements StorageProvider {
  ...
  private getEndpoint() {
    ...
    return this.stripTrailingBucket(endpoint);
  }

  private stripTrailingBucket(endpoint: string) {
    const normalized = endpoint.replace(/\/+$/, '');
    const bucket = this.configs.bucket?.replace(/^\/+|\/+$/g, '');
    if (!bucket) return normalized;
    return normalized.endsWith(`/${bucket}`)
      ? normalized.slice(0, -bucket.length - 1)
      : normalized;
  }
  ...
  getPublicUrl = (options: { key: string; bucket?: string }) => {
    ...
    return this.configs.publicDomain
      ? `${this.configs.publicDomain.replace(/\/+$/, '')}/${uploadPath}/${options.key}`
      : url;
  };
