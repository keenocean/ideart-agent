...
export function getSettingGroups(): SettingGroup[] {
  return [
    ...
    },
    {
      ...
      description: 'Which provider serves image generation',
      tab: 'ai',
    },
    {
...
export function getSettings(): Setting[] {
  return [
    ...
    },
    {
      ...
      type: 'textarea',
      placeholder: 'Create and refine AI video clips through conversation.',
      group: 'appinfo',
      tab: 'general',
    },
    {
    ...
    {
      name: 'signup_ip_daily_limit',
      title: 'Signups per IP / day',
      type: 'number',
      placeholder: '5',
      tip: 'Caps how many accounts one address may open in 24 hours, so the signup credit grant cannot be farmed. 0 disables the cap; blank uses the default of 5.',
      group: 'credit',
      tab: 'general',
    },
    ...
    // ─── AI / OpenAI ─────────────────────────────────────────────────
    {
      ...
      tip: 'Include the version segment — the client appends /chat/completions.',
      group: 'openai',
      tab: 'ai',
    },
    {
    ...
    // ─── AI / Anthropic ──────────────────────────────────────────────
    {
      ...
      tip: 'Host only — the client appends /v1/messages. Gateways expose this protocol on their own path (gRouter: /api/anthropic).',
      group: 'anthropic',
      tab: 'ai',
    },
    {
    ...
    // ─── AI / Chat model ─────────────────────────────────────────────
    {
      ...
      tip: 'Auto uses OpenAI when its API key is set, otherwise Anthropic. Credentials come from the provider cards below.',
      group: 'agent_llm',
      tab: 'ai',
      defaultValue: 'auto',
    },
