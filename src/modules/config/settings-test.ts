...
async function testR2(
  ...
  const safeName = (inputs.filename || 'ideart-settings-test.txt').replace(
    /[^a-zA-Z0-9._-]/g,
    '_'
  );
  const key = `settings-test/${Date.now()}-${safeName}`;
  const body = Buffer.from(
    `Ideart settings test\nTimestamp: ${new Date().toISOString()}\n`,
    'utf-8'
  );
...
async function testOpenAI(
  ...
  const baseUrl =
    normalizeOpenAIBaseUrl(configs.openai_base_url) ||
...
async function testAnthropic(
  ...
  const baseUrl =
    normalizeAnthropicBaseUrl(configs.anthropic_base_url) ||
