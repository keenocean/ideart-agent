...
import { hasMarketingHomeProjection } from '@/content/marketing';
...
describe('catalog first-fold performance policy', () => {
  ...
  it('keeps JSON-driven homepage marquee media empty until published', async () => {
    expect(locales.map((locale) => hasMarketingHomeProjection(locale))).toEqual(
      locales.map(() => false)
    );
  });

  it('keeps published homepage marquee media within its eager budget', async () => {
    const publishedLocales = locales.filter((locale) =>
      hasMarketingHomeProjection(locale)
    );
    if (publishedLocales.length === 0) {
      expect(publishedLocales).toEqual([]);
      return;
    }
  ...
  it('keeps catalog LCP text independent from Google network fonts', async () => {
