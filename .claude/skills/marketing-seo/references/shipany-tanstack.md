...
- Match the ShipAny homepage contract: `src/routes/index.tsx` explicitly composes stable flat Blocks, and those Blocks statically read the complete project-specific `landing.*` homepage content from `messages/<locale>.json`. Standard project replacement changes `landing.*`, the current repository's asset inventory, and R2 objects without rewriting the homepage Route/Blocks/Components. Change code only for a new interaction or information architecture.
- Represent homepage media selection with finite static keys such as `landing.hero.media_asset_id`, `landing.examples.item_1.asset_id`, localized alt/caption, and `landing.seo.og_asset_id`. Blocks must call each known message key statically; do not use `tDynamic` or a runtime-built key. Resolve non-empty asset IDs against the current repository's inventory at build time and emit only referenced media into the generated home projection.
- `messages/<locale>.json` also contains short UI/metadata messages; known keys use static Paraglide access. Public marketing graphs must not use `tDynamic` or runtime-built message keys.
- For the 100+ page profile, repository-edited scalable long-form content lives under `messages/marketing/**`: directories, `tools/<entityId>/<locale>.json`, `models/<entityId>/<locale>.json`, and `assets.json`. Do not create `messages/marketing/home`; homepage copy remains `landing.*`. Scalable content is excluded from Paraglide and route/client globs, schema-validated at build time, and published as immutable external content releases. Do not extend legacy `src/content/**/pages/*.ts` lazy chunks for new bulk pages.
...
- Components receive media props and never read storage configuration or concatenate the domain. Keep assetId plus absolute URL/kind/MIME/dimensions/bytes/poster in the current repository's `messages/marketing/assets.json`; never write a derived project's inventory into the ShipAny template checkout or another repository. Keep homepage assetId slots and localized alt/caption in `landing.*`; keep detail-page alt/caption in the locale page JSON. Resolve only referenced assets into a small generated home/page payload so runtime never downloads the global asset registry. Never put R2 credentials in Catalog, source JSON, client bundles, or release manifests.
...
- Use immutable object keys such as `marketing-content/releases/<releaseId>/pages/<kind>/<entityId>/<locale>.json`, plus release-scoped directory projections, a generated `projections/home/<locale>.json` for featured Catalog cards/resolved media, and a lightweight manifest. The home projection is generated infrastructure, not an authored homepage-content JSON.
...
- Homepage copy comes from statically accessed `landing.*`. Homepage featured Catalog cards/resolved media, directories, Related, sitemap, llms, detail availability, and language targets consume the same pinned release projections. They may not scan source JSON independently or read a mutable latest release.
