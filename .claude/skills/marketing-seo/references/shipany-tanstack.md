...
- Match the ShipAny homepage contract: `src/routes/index.tsx` renders the closed section registry in `product/home.json`, and those Blocks statically read the complete project-specific `landing.*` homepage content from `product/messages/<locale>.json`. Standard product replacement changes the Product Pack and R2 objects without rewriting the homepage Route/Blocks/Components. Change code only for a new interaction or information architecture.
...
- `product/messages/<locale>.json` also contains short UI/metadata messages; known keys use static Paraglide access. Public marketing graphs must not use `tDynamic` or runtime-built message keys.
- For the 100+ page profile, repository-edited scalable long-form content lives under `product/marketing/**`: directories, `tools/<entityId>/<locale>.json`, `models/<entityId>/<locale>.json`, and `assets.json`. Do not create `product/marketing/home`; homepage copy remains `landing.*`. Scalable content is excluded from Paraglide and route/client globs, schema-validated at build time, and published as immutable external content releases. Do not extend legacy `src/content/**/pages/*.ts` lazy chunks for new bulk pages.
